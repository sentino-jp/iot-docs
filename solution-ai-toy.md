...
| **多角色切换** | 通过 App 操作或 NFC 卡片（可选）切换不同 AI 角色 |
| **角色自定义** | 家长通过 App 自定义 AI 角色的人设、声音 |
| **AI 设备控制** | 对话中通过语音指令控制玩偶硬件（如表情、动作、LED 灯光、音量） |
...
```mermaid
graph TD
    Toy["AI 玩偶<br/>（按键 + 扬声器 + 麦克风 + NFC 可选）"]
...
> NFC 角色切换是可选增强功能，适用于配备 NFC 硬件的设备。未配备 NFC 的设备可通过 App 管理角色切换，不影响核心语音对话能力。

NFC 让儿童通过实体卡片与设备互动，是一种直观的交互方式。
