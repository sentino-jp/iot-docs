...
| 能力 | 说明 |
|---|---|
| **实时语音对话** | 儿童与 AI 角色自然对话，端到端延迟 < 400ms |
...
graph TD
    ...
    Cloud["Sentino 云端<br/>（设备管理 + AI Agent）"]
...
- **反馈设计**：放置卡片后通过提示音 + LED 指示切换成功（如 "你好，我是故事大王！"）
- **默认角色**：设备应有一个默认角色，无卡片时使用
- **卡片管理**：家长可通过 App 查看已有的 NFC 卡片和对应角色
- **扩展包**：可作为增值产品销售额外的 NFC 角色卡片
...
| **音量控制** | 侧面旋钮或 +/- 按键 |
| **状态指示** | LED 灯环：蓝色=对话中、绿色=在线、橙色=配网中、红色=离线 |
...
| **响应速度** | 从按键到 AI 开始回复，目标 < 3 秒 |
| **打断处理** | 支持用户随时按键打断 AI 回复 |
| **超时处理** | 长时间无语音活动（如 30 秒），自动结束对话并播放提示 |
...
| 音频 Codec | 支持 16kHz / 16bit 采样 |
| 麦克风 | 数字麦克风，支持降噪 |
| 扬声器 | 3W+ 功率 |
| NFC 读卡器 | ISO 14443A/B |
| BLE | BLE 4.2+，支持 GATT |
| 存储 | NVS 分区用于三元组和配置存储 |
...
| 环节 | 说明 |
|---|---|
| **NFC 标识分配** | 每张 NFC 卡片写入唯一标识（`nfcIdentifier`） |
| **角色绑定** | 在 Sentino 后台将 NFC 标识与智能体关联 |
| **卡片设计** | 卡面印刷角色形象和名称 |
| **包装** | 随设备附带基础角色卡片，扩展卡片可单独销售 |
