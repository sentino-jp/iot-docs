...
> **注意**：App 端扫描时按广播名称过滤设备（默认 `"RY"`，具体产品可自定义）。广播中的 Service UUID (`0xA101`) 用于标识 Sentino 设备并携带产品 ID。实际 GATT 数据通信使用不同的 Service 和 Characteristic UUID。
...
**相关文档**：[App 端集成指南](../guides/guide-app.md) | [设备端集成指南](../guides/guide-device.md) | [架构与概念](../architecture.md)
