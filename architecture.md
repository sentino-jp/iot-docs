...
> **平台定位**：Sentino IoT 是端到端 IoT 方案——你的 App / 设备直接调 Sentino 云，**无需客户自建后端**。本文档只覆盖「物」的部分（设备接入、配网、MQTT、OTA、智能体绑定）；自定义 AI Agent 行为（LLM/TTS 编排、记忆、工作流、自定义工具）属于 Sentino Agent 平台范畴，请联系 Sentino 团队获取相关文档。
...
| **MAC** | Sentino 平台业务侧的设备身份串 | 相当于"会员卡号" |

> **`MAC` 字段不参与任何网络通信**：它只是云端用于标识设备的字符串，跟设备实际通信用的 WiFi MAC、BLE MAC（SoC 模块自带）是不同字段。MQTT 协议链路上仅出现 `UUID` / `KEY` / `PID`，`MAC` 完全不出现。

可选周边物料：**Barcode**（条码）。当产品选用"条码绑定"路径（[REST §4.6](reference/ref-rest-api.md#46-条形码配网)）时，Sentino 会一并交付 Barcode，工厂将其印刷到设备外壳供 App 扫码绑定；其他绑定路径（BLE 配网、4G 5 位绑定码）不需要外壳条码。
...
| 配置项 | 用途 | 示例 |
|--------|------|------|
| `app_id` | 业务层应用标识，所有 REST API 请求头必带 | `krkfvb4s5e91hq` |
| 渠道标识符 (`channel_identifier`) | 标识 App 的发布渠道 | `kfvb4s5e` |
...
| 阶段 | 参与者 | 关键动作 |
|---|---|---|
| **出厂** | 工厂 | 将三元组 (UUID/KEY/MAC) 烧录到 NVS。**Barcode 印刷到外壳为可选工艺**，仅当产品选用条码绑定路径时需要 |
