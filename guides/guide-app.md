...
| `channel_identifier` | `kfvb4s5e` | 渠道标识 |
| `package_name` | `jp.sentino.general` | 包名 |
| `app_id` | `krkfvb4s5e91hq` | 应用 ID |
...
| BLE 参数 | 值 |
|---|---|
| 广播名称 | `"RY"`（默认值，具体产品可自定义） |
| 广播数据中包含 | Service UUID `0xA101` + 产品 ID (PID) |
| 扫描应答数据包含 | 设备 UUID/MAC、配网状态标志 |

App 扫描 BLE 设备，按广播名称过滤（默认 `"RY"`，具体名称以产品定义为准），连接后：
