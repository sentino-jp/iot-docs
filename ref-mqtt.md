...
> **前置知识**：请先阅读 [架构与概念](./architecture.md) 中的"MQTT 通信模型"章节。
...
**变量说明：**
- `${pid}` — 产品 ID（Sentino 分配，同产品所有设备共享）
...
> 设备连接后需**立即订阅** `report_response` 和 `issue`。
...
| 字段 | 类型 | 必填 | 说明 |
|:---|:---|:---|:---|
| `id` | string | 是 | 消息唯一 ID，建议 UUID v4。同一设备短时间内不可重复，否则消息被忽略 |
...
| 字段 | 类型 | 说明 |
|:---|:---|:---|
| `properties` | object | 属性键值对集合，key 为物模型中的 identifier |
...
| 字段 | 类型 | 说明 |
|:---|:---|:---|
| `appId` | string | Agora 应用 ID，用于初始化 SDK |
| `rtcToken` | string | RTC 通道令牌（有时效），用于加入频道鉴权 |
| `channelName` | string | RTC 通道名称 |
| `uid` | int | RTC 成员 ID |

> Token 有时效性，获取后应尽快加入频道。
...
**设备回复 data：** 无额外字段。设备收到后开始下载，通过 `ota_progress` 上报进度。
...
| `wifiList` | array | 可用网络列表，每项含 `ssid` 和 `rssi`（信号强度 dBm） |
...
**相关文档**：[架构与概念](./architecture.md) | [快速入门 — 设备端](./quickstart-device.md)
